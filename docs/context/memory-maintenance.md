...
| 2026-07-21 | unknown-surface | CI/release automation | Release/install proof exists in local Node and Go commands, but `.github/workflows/` is empty and the durable owner of external CI is not documented here. | Resolved 2026-08-05: local-only proof is **intentional**. `kbcheck` is the release authority; hosted CI is deliberately not used. `.github/workflows/` stays empty by design. README corrected — it had claimed a cross-platform CI matrix and release workflows that never existed. | closed |
