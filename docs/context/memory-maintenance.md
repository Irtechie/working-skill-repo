...
| 2026-05-29 | bloat-risk | hot-path skills | Several hot-path skills exceed 400 lines, but the threshold is a review signal rather than an automatic trim target. | Keep always-needed routing/gate content inline; move only genuinely optional or rarely used material into lazy references. Current long hot-path skills are intentionally allowlisted. | closed |
...
| 2026-06-10 | minimality | agent surface | `kbcheck minimality` reports 11 unproven agents and one trim candidate, but the report is static-only and not a deletion approval. | Treat as cold-storage review input; require human approval and runtime proof before deleting agents. | open |
