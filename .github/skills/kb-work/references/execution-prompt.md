...
14. Before any observed write exceeds the plan-run claim, return it for
    coordinator expansion. A failed expansion requeues before the write.
15. Stage only files changed for this slice.
16. Commit in the current plan-run worktree only if the user authorized commits.
    Do not merge, reset, stash, clean, rebase, or amend another slice's commit.
17. Return the current commit SHA, observed writes, and a machine-readable proof
...
18. Renew the slice lease before handing back. The coordinator releases it only
